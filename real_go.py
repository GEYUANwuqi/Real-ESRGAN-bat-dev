...
def rename_frames(directory):
    ...
    for old_file in files:
        old_num = int(pattern.match(old_file).group(1))
        new_num_str = f"{new_start:04d}"
        old_path = Path(directory) / old_file
        new_file = f"frame_{new_num_str}.png"
        new_path = Path(directory) / new_file
        os.rename(old_path, new_path)
        new_start += 1 # 定义条件重命名视频帧函数
...
print(
    "\n简易使用指南:"
    "\n1.anime模型对二次元图片的超分有优化,ganx模型用于三次元超分,带video的模型专用于视频超分(更多模型说明请查看“脚本使用说明”)"
    "\n2.在输入文件名称时,无需输入文件后缀"
...
models = input("请选择模型(回车默认为animehqx4): ").lower() or "animehqx4"
...
# 图片或视频的选择
if models == "animex4" or models == "ganx4" or models == "animehqx4" or models == "generalx4" :
    ...
    if video_name == "no":
        ...
        rename_frames(tmp_video_frame_file)
